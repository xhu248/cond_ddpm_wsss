...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, time_emb_dim=128):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None, emb=None):
        ...
        if emb is not None:
            ...
            if len(emb_out.shape) < len(x.shape):
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix, emb):
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, skip, S, H, W, emb):
        ...
        x = self.blocks[0](x, attn_mask, skip=skip, x_up=x_up, emb=emb)
        for i in range(self.depth - 1):
            x = self.blocks[i + 1](x, attn_mask, emb)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, emb):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, S, H, W, x, Ws, Wh, Ww = layer(x, Ws, Wh, Ww, emb)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, skips, emb):
        ...
        for i in range(self.num_layers)[::-1]:
            ...
            x, S, H, W, = layer(x, skips[i], S, H, W, emb)
...
class nnFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[64, 128, 128],
        ...
        window_size = window_size
        self.model_down = Encoder(pretrain_img_size=crop_size, window_size=window_size, embed_dim=self.embed_dim,
                                  patch_size=patch_size, depths=depths, num_heads=num_heads, in_chans=input_channels,
                                  time_emb_dim=time_emb_dim)
        self.decoder = Decoder(pretrain_img_size=crop_size, embed_dim=self.embed_dim, window_size=window_size[::-1][1:],
        ...
        if self.do_ds:
            ...
            for i in range(len(depths) - 1):
                self.final.append(final_patch_expanding(self.embed_dim * 2 ** i, num_classes, patch_size=patch_size))

        else:
            self.final.append(final_patch_expanding(self.embed_dim, num_classes, patch_size=patch_size))

        self.final = nn.ModuleList(self.final)

    def forward(self, x, timesteps, y=None):
        emb = self.time_embed(timestep_embedding(timesteps, self.time_emb_dim))

        seg_outputs = []
        skips = self.model_down(x, emb)
        neck = skips[-1]

        out = self.decoder(neck, skips, emb)
    ...
    def forward_features(self, x, timesteps):
        emb = self.time_embed(timestep_embedding(timesteps, self.time_emb_dim))
